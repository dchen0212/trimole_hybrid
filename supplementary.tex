...
Supplementary Tables A--C and Supplementary Figures S1--S2 provide compact PDF summaries. Supplementary Tables S1--S24 and S4e provide the complete machine-readable record. Table S1 reports the corrected 22-task benchmark; Tables S2, S13 and S15 record task configurations and selection evidence; Tables S3 and S14 record the fixed public-reference snapshot; Tables S4, S4e and S23 record scale-aware ablations and selection stability; Tables S7--S10 and S17 record molecule-level case-study checks; Tables S11 and S19 record split, source, endpoint and applicability provenance; Tables S20--S20d record target-wide cross-task filtering, residual similarity and corrected family-transfer scores; Tables S21--S22 retain the earlier controlled sensitivity analyses; Tables S24g--S24j provide the primary strict common-pool comparison and hierarchical uncertainty analysis, while Tables S24--S24f retain the preceding candidate-eligibility and expanded-pool audits; Table S16 records metric interpretation; and Table S18 records the software environment.
...
\end{minipage}
\caption{Leakage-controlled development and evaluation protocol. The test split can be featurized without labels for final prediction, but its labels are inaccessible to selection and refitting code. Before stage-specific fitting, family-transfer candidates construct one label-free target universe from the already defined train, validation and test structures; the same exact-identity and Morgan-Tanimoto $\geq 0.90$ source filter is then used for selection and final refitting. \textbf{Alt text:} Five vertically ordered boxes separate frozen splitting and feature extraction, target-wide cross-task filtering, base-model fitting, validation-only decisions, frozen train-plus-validation refitting and final test-only scoring.}
...
Supplementary Tables S20--S20d report pairwise molecular overlap within the CYP-substrate and clearance families using the connectivity layer of an RDKit-generated InChIKey, the single target-wide exclusion universe, residual Bemis--Murcko scaffold overlap, Morgan-fingerprint Tanimoto similarity, rows removed by exact and high-similarity filters, corrected five-seed scores and provenance. The previously reported CYP3A4-substrate and hepatocyte-clearance family-transfer results were invalidated and replaced because their source pools contained target-test identities. The new rule is independent of target split assignment: all target structures are declared before fitting and the same cross-task mask is used in selection and refitting.
...
Supplementary Tables S24g--S24j contain the primary strict fairness experiment. All eight methods use the same nine candidate families, five seeds and official splits. The constrained task-wise selector chooses among six predeclared recipe rules, while common-pool AutoML chooses among six predeclared meta-model configurations; both therefore have the same input pool, candidate count and validation-selection budget. Deterministic global, per-task, top-$k$, uniform and stacking rules require no additional search. The frozen selection manifest is written before test predictions are opened. Across 880 method--task--seed scores, validation top-3 and top-2 averaging achieved mean ranks of 2.91 and 3.00; the task-wise selector ranked third at 3.73 and was best on two tasks. Relative to per-task single-family selection it had 11 positive point estimates, five FDR-significant gains and no significant loss. Relative to common-pool AutoML it had 13 positive point estimates, three significant gains and three significant losses.

Table S24i uses 10,000 hierarchical seed-and-sample bootstrap replicates. Each replicate resamples the five paired seeds and test molecules before recomputing the official metric from the resampled seed-average prediction. One Benjamini--Hochberg correction covers all 154 task--comparator tests. These intervals incorporate training-seed and finite-test-sample variation. The mixed positive and negative findings do not support universal superiority; they support the narrower claim that endpoint-specific validation can help relative to a single selected family, while simple top-$k$ averaging or AutoML is preferable for other endpoints.
...
S20 & \path{Table_S20_cross_task_leakage_audit.csv} & Original pairwise identity audit, supplemented by Tables S20b--S20d for residual scaffold/Tanimoto overlap, target-wide exact/high-similarity filtering and corrected five-seed scores. \\
...
S22 & \path{Table_S22_uncertainty_and_subgroups.csv} & Historical sample-only bootstrap and 132 subgroup records; BBB is explicitly not interpreted as seed-aware evidence. \\
...
S24 & \path{Table_S24_expanded_candidate_pool_controls.csv} & Candidate-pool construction and eligibility audit in S24--S24f; the primary eight-method strict common-pool runs, summaries, hierarchical bootstrap and validation selections are provided in Tables S24g--S24j. \\
\bottomrule
\end{longtable}
\rowcolors{2}{}{}
}
