...
def suggest_params(trial: optuna.Trial, group_name: str) -> dict:
    ...
    if group_name in {"veith", "classification"}:
        common.update({
...
def build_cmd(group_name: str, params: dict, out_dir: Path, tasks: list[str]) -> list[str]:
    ...
    if group_name in {"veith", "classification"}:
        cmd += ["--fusion-type", params["fusion_type"]]
...
def run_study(group_name: str, n_trials: int):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        out_dir = Path("results/model_log/admet17_ex_top1_optuna") / group_name / f"trial_{trial.number:03d}"
    ...
    study = optuna.create_study(
        study_name=f"admet17_ex_top1_{group_name}",
        direction="maximize",
    ...
    out_path = Path("results/model_log/admet17_ex_top1_optuna") / f"{group_name}_best.json"
...
def main():
    ...
    parser.add_argument("--group", choices=["veith", "classification", "regression", "all"], default="all")
    parser.add_argument("--n-trials", type=int, default=8)
    args = parser.parse_args()

    groups = ["veith", "classification", "regression"] if args.group == "all" else [args.group]
    for g in groups:
