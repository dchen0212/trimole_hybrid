...
OUT_DIR = PROJECT / "results" / "model_log" / "hia_xgb_refine_maplight"
...
def eval_cls(y_true, y_prob):
    ...
    return {
        "valid_or_test_auc": float(roc_auc_score(y_true, y_prob)),
    ...
    for seed in SEEDS:
        ...
        rows.append({
            **base_cfg,
            "seed": seed,
            "valid_auc": valid_auc,
            "test_auc": test_m["valid_or_test_auc"],
            "test_auprc": test_m["auprc"],
            "test_acc": test_m["acc"],
        })

        pd.DataFrame(rows).to_csv(OUT_DIR / "hia_xgb_refine_maplight_live.csv", index=False)

df = pd.DataFrame(rows)
df.to_csv(OUT_DIR / "hia_xgb_refine_maplight_summary.csv", index=False)
...
agg.to_csv(OUT_DIR / "hia_xgb_refine_maplight_agg.csv", index=False)

print("\n=== HIA MAPLIGHT-XGB AGG ===")
print(agg.head(50).to_string(index=False))
print("\nSaved:")
print(" -", OUT_DIR / "hia_xgb_refine_maplight_summary.csv")
print(" -", OUT_DIR / "hia_xgb_refine_maplight_agg.csv")
