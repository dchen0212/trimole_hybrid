...
OUT_DIR = ROOT / "results/model_log/cyp2d6_txg_refine_v2"
OUT_DIR.mkdir(parents=True, exist_ok=True)

TASK = "cyp2d6_veith"
CENTER = (0.157895, 0.473684, 0.368421)
STEP = 0.02
...
vt = ROOT / "results/model_log/fusion_inputs_valid_trimole_22tasks/cyp2d6_veith_valid_predictions.csv"
vx = ROOT / "results/model_log/xgb_baseline_22tasks_with_valid/run_xgb_baseline_22tasks_with_valid/cyp2d6_veith_valid_predictions.csv"
vg = newest(list((ROOT / "results/model_log/gnn_v2_22tasks/cyp2d6_veith").rglob("valid_predictions.csv")))

tt = newest(list((ROOT / "results/model_log/final_best_v4_runs/cyp2d6_veith").rglob("test_predictions.csv")))
tx = newest([p for p in (ROOT / "results/model_log").rglob("cyp2d6_veith_test_predictions.csv") if "xgb" in str(p).lower() or "baseline" in str(p).lower()])
tg = newest(list((ROOT / "results/model_log/gnn_v2_22tasks/cyp2d6_veith").rglob("test_predictions.csv")))
...
detail.to_csv(OUT_DIR / "cyp2d6_txg_refine_v2_detail.csv", index=False)
summary.to_csv(OUT_DIR / "cyp2d6_txg_refine_v2_summary.csv", index=False)

print("\n=== SUMMARY ===")
print(summary.to_string(index=False))
print("\nSaved:", OUT_DIR / "cyp2d6_txg_refine_v2_summary.csv")
print("Saved:", OUT_DIR / "cyp2d6_txg_refine_v2_detail.csv")
