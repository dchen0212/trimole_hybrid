...
for combo in itertools.product(*vals):
    ...
    for seed in SEEDS:
        ...
        if proc.returncode != 0 or not cand:
            ...
            pd.DataFrame(rows).to_csv(OUT_ROOT / "bbb_refine_live.csv", index=False)
            continue
...
df.to_csv(OUT_ROOT / "bbb_refine_summary.csv", index=False)
...
agg.to_csv(OUT_ROOT / "bbb_refine_agg.csv", index=False)

print("\n=== AGG ===")
print(agg.to_string(index=False))
print("\nSaved:")
print(" -", OUT_ROOT / "bbb_refine_summary.csv")
print(" -", OUT_ROOT / "bbb_refine_agg.csv")
