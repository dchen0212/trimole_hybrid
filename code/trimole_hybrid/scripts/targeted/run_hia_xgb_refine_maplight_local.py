...
OUT_DIR = PROJECT / "results" / "model_log" / "hia_xgb_refine_maplight_local"
...
for combo in itertools.product(*vals):
    ...
    for seed in SEEDS:
        ...
        cfg.update({
            ...
            "n_jobs": 8,
            "eval_metric": "auc",
            # 留一个很大的上限，让 early stopping 自己找最佳轮数
            "n_estimators": max(base_cfg["n_estimators"], 2000),
        })

        print(f"[{cur}/{total}] {base_cfg} seed={seed}")
        ...
        # sklearn 接口里最好轮数会自动用于 predict / predict_proba
        ...
        pd.DataFrame(rows).to_csv(OUT_DIR / "hia_xgb_refine_maplight_local_live.csv", index=False)

df = pd.DataFrame(rows)
df.to_csv(OUT_DIR / "hia_xgb_refine_maplight_local_summary.csv", index=False)
...
agg.to_csv(OUT_DIR / "hia_xgb_refine_maplight_local_agg.csv", index=False)

print("\n=== HIA MAPLIGHT-XGB LOCAL AGG ===")
print(agg.head(50).to_string(index=False))
print("\nSaved:")
print(" -", OUT_DIR / "hia_xgb_refine_maplight_local_summary.csv")
print(" -", OUT_DIR / "hia_xgb_refine_maplight_local_agg.csv")
