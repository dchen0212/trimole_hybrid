...
BASES = [
    Path("results/model_log/final_best_of_all_22_v7_selected_with_config.csv"),
...
PATCH = Path("results/model_log/gated_3d_downweight_all_22/best_by_task.csv")
...
OUT1 = Path("results/model_log/final_best_of_all_22_3d_downweight_full.csv")
OUT2 = Path("results/model_log/final_best_of_all_22_3d_downweight_full_with_config.csv")
OUT3 = Path("results/model_log/final_best_3d_downweight_full_patch_only.csv")
OUT4 = Path("results/model_log/final_best_3d_downweight_full_summary.json")
...
for _, row in patch.iterrows():
    ...
    old_mae = merged.at[idx, "test_mae"] if "test_mae" in merged.columns else np.nan
    new_mae = row.get("test_mae", np.nan)
    ...
    if pd.notna(new_auc):
        ...
        old_val = old_auc
        new_val = new_auc
        delta = float(new_auc - old_auc) if pd.notna(old_auc) else np.nan
    elif pd.notna(new_mae):
        better = pd.notna(new_mae) and (pd.isna(old_mae) or float(new_mae) < float(old_mae))
        metric_name = "MAE"
        old_val = old_mae
        new_val = new_mae
        delta = float(new_mae - old_mae) if pd.notna(old_mae) else np.nan
    else:
        better = False
        metric_name = "UNKNOWN"
        old_val = np.nan
    ...
    if better:
        for c in [
            "primary_metric", "best_valid_primary", "primary_metric_name",
            "test_auc", "test_auprc", "test_acc",
            "test_mae", "test_rmse", "test_spearman", "loss_type"
        ]:
        ...
        if "rerun_results_file" in merged.columns:
            merged.at[idx, "rerun_results_file"] = row["results_file"]
        if "rerun_run_name" in merged.columns:
...
if len(selected):
    print(selected[["task", "metric_name", "old_value", "new_value", "delta_new_minus_old", "cfg_name"]].to_string(index=False))
else:
    print("None")
