...
OUT = ROOT / "results/model_log/clhepa_rank_push"
...
def make_grid():
    vals_t = np.arange(0.40, 0.86 + 1e-9, 0.01)
    vals_x = np.arange(0.00, 0.16 + 1e-9, 0.01)
    out = []
...
rv_t, rv_x, rv_g = rankdata(p_tv), rankdata(p_xv), rankdata(p_gv)
rt_t, rt_x, rt_g = rankdata(p_tt), rankdata(p_xt), rankdata(p_gt)

best = None
for wt, wx, wg in make_grid():
    pv = wt * rv_t + wx * rv_x + wg * rv_g
    vv = metric(y_tv, pv)
    if best is None or vv > best["best_valid_score"]:
        pt = wt * rt_t + wx * rt_x + wg * rt_g
        ts = metric(y_tt, pt)
        best = {
            "task": TASK,
            "metric": "Spearman",
...
df.to_csv(OUT / "clhepa_rank_push.csv", index=False)
