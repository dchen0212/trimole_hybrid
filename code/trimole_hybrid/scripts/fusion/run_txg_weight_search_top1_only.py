...
TASKS = {
    ...
    "clearance_hepatocyte_az": {"metric": "Spearman", "center": (0.76, 0.18, 0.06)},
    "cyp2d6_veith": {"metric": "AUPRC", "center": (0.20, 0.44, 0.36)},
}

OUT_DIR = ROOT / "results/model_log/txg_weight_search_top1_only"
...
def make_grid(center, radius=0.06, step=0.01):
    ct, cx, cg = center
...
pd.DataFrame(rows).to_csv(OUT_DIR / "top1_only_summary.csv", index=False)
