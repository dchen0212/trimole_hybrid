...
OUT_CSV = RERUN_DIR / "benchmark_clean_final_master_v9.csv"
OUT_TXT = RERUN_DIR / "benchmark_clean_final_master_v9.txt"

PATCH_TASK = "bbb_martins"
PATCH_METRIC = "AUROC"
PATCH_SCORE = 0.923311
PATCH_SOURCE = "bbb_model_sweep_top1_push"
PATCH_NOTE = "replaced by v9 single-task top1 push; gated hidden_dim=256 dropout_head=0.2 dropout_proj=0.2 lr=1e-4 wd=1e-5 seed=123"

df = pd.read_csv(SRC).copy()

df["v9_patch_applied"] = "no"
df["v9_patch_source"] = ""
df["v9_test_score"] = np.nan
...
df.loc[mask, "final_source"] = "v9_patch"
df.loc[mask, "final_note"] = PATCH_NOTE + f"; previous score={old_score}"
df.loc[mask, "v9_patch_applied"] = "yes"
df.loc[mask, "v9_patch_source"] = PATCH_SOURCE
df.loc[mask, "v9_test_score"] = PATCH_SCORE

df = df.sort_values("task").reset_index(drop=True)
df.to_csv(OUT_CSV, index=False)

with open(OUT_TXT, "w", encoding="utf-8") as f:
    f.write(df[df["v9_patch_applied"]=="yes"][[
        "task","primary_metric_name","primary_metric","v9_patch_source","v9_test_score","final_note"
    ]].to_string(index=False))
    f.write("\n\n=== source counts ===\n")
    f.write(df["final_source"].value_counts().to_string())
    f.write("\n")

print(df[df["v9_patch_applied"]=="yes"][[
    "task","primary_metric_name","primary_metric","v9_patch_source"
]].to_string(index=False))
print("\n=== source counts ===")
