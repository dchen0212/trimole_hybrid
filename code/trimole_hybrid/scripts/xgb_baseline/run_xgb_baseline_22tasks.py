...
# TDC ADMET Group official metrics
TASK_CONFIG = {
...
def eval_cls(y_true, y_prob, official_metric: str):
    y_pred = (y_prob >= 0.5).astype(int)
    auc = float(roc_auc_score(y_true, y_prob))
    auprc = float(average_precision_score(y_true, y_prob))
    acc = float(accuracy_score(y_true, y_pred))
    primary = auc if official_metric == "AUROC" else auprc
    return {
        "test_auc": auc,
        "test_auprc": auprc,
        "test_acc": acc,
...
def eval_reg(y_true, y_pred, official_metric: str):
    mae = float(mean_absolute_error(y_true, y_pred))
    sp = float(spearmanr(y_true, y_pred).statistic)
    primary = mae if official_metric == "MAE" else sp
    return {
        "test_mae": mae,
        "test_spearman": sp,
        "primary_metric": float(primary),
...
def fit_one(task_dir: Path, task: str, seed: int):
    task_type, official_metric = TASK_CONFIG[task]
    ...
    if task_type == "classification":
        ...
        valid_primary = (
            ...
            if official_metric == "AUROC"
            else average_precision_score(y_valid, p_valid)
        )
        ...
        row.update(eval_cls(y_test, p_test, official_metric))
    ...
    valid_primary = (
        mean_absolute_error(y_valid, p_valid)
        if official_metric == "MAE"
        else spearmanr(y_valid, p_valid).statistic
    )
    ...
    row.update(eval_reg(y_test, p_test, official_metric))
    return row, y_test, p_test
...
def main():
    ...
    run_dir = out_root / "run_xgb_baseline_22tasks"
    ...
    for task in tasks:
        try:
            ...
            pd.DataFrame({"task": task, "y_true": y_true, "y_pred": y_pred}).to_csv(
