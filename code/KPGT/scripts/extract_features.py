...
def parse_args():
    ...
    parser.add_argument("--batch_size", type=int, default=128, help="Batch size for feature extraction (default 128, good for H100)")
...
def extract_features(args):
    ...
    loader = DataLoader(mol_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, drop_last=False, collate_fn=collator)
    model = LiGhT(
    ...
    model.load_state_dict({k.replace('module.',''):v for k,v in torch.load(args.model_path, map_location="cpu").items()})
    fps_list = []
