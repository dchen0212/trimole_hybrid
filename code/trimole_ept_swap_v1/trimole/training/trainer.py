...
def _resolve_ept_embedding_paths(task_dir: Path):
    em_map = task_dir / "embeddings_ept"
    cand = {
        "train": em_map / "train_ept.npy",
        "valid": em_map / "valid_ept.npy",
        "test": em_map / "test_ept.npy",
    }
    missing = [str(x) for x in cand.values() if not x.exists()]
    if missing:
        raise FileNotFoundError(
            f"EPT embeddings are required for the EPT project but missing under {em_map}: {missing}"
        )
    return cand
...
from trimole.training.losses import (
    AUROCMarginLoss,
    AUPRCSurrogateLoss,
    FocalLoss,
    SpearmanLoss,
...
class TrainConfig:
    ...
    modalities: str = "all"  # all | chemberta | ept | kpgt
    ...
    # loss_type:
    # - "auto": default to weighted CrossEntropy (stable); AUPRC tasks may override.
    # - "weighted_ce": weighted CrossEntropyLoss (optionally with pos_weight/neg_weight).
    # - "ap_surrogate": focal + pairwise ranking surrogate for AUPRC-oriented tasks.
    loss_type: str = "auto"
...
def sample_training_modality_mask(
    base_mask: Optional[Tuple[bool, bool, bool]],
    dropout_prob: float,
) -> Optional[Tuple[bool, bool, bool]]:
    if dropout_prob <= 0:
        return base_mask

    active = [True, True, True] if base_mask is None else [bool(x) for x in base_mask]
    active_idx = [i for i, on in enumerate(active) if on]
    if len(active_idx) <= 1:
        return tuple(active)
    if float(np.random.rand()) >= float(dropout_prob):
        return tuple(active) if base_mask is not None else None

    drop_idx = int(np.random.choice(active_idx))
    active[drop_idx] = False
    return tuple(active)


def sanitize_and_normalize_ept(
    train_arr: np.ndarray,
    valid_arr: np.ndarray,
    test_arr: np.ndarray,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict[str, float]]:
    train_arr = np.nan_to_num(train_arr.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    valid_arr = np.nan_to_num(valid_arr.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    test_arr = np.nan_to_num(test_arr.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)

    finite = np.abs(train_arr[np.isfinite(train_arr)])
    clip_value = float(np.percentile(finite, 99.9)) if finite.size else 50.0
    clip_value = float(min(max(clip_value, 50.0), 1e4))

    train_arr = np.clip(train_arr, -clip_value, clip_value)
    valid_arr = np.clip(valid_arr, -clip_value, clip_value)
    test_arr = np.clip(test_arr, -clip_value, clip_value)

    feat_mean = train_arr.mean(axis=0, dtype=np.float64).astype(np.float32)
    feat_std = train_arr.std(axis=0, dtype=np.float64).astype(np.float32)
    feat_std = np.where(feat_std < 1e-6, 1.0, feat_std).astype(np.float32)

    train_arr = (train_arr - feat_mean) / feat_std
    valid_arr = (valid_arr - feat_mean) / feat_std
    test_arr = (test_arr - feat_mean) / feat_std

    train_arr = np.nan_to_num(train_arr, nan=0.0, posinf=0.0, neginf=0.0)
    valid_arr = np.nan_to_num(valid_arr, nan=0.0, posinf=0.0, neginf=0.0)
    test_arr = np.nan_to_num(test_arr, nan=0.0, posinf=0.0, neginf=0.0)

    stats = {
        "ept_clip_value": clip_value,
        "ept_train_abs_p99_9": clip_value,
    }
    return train_arr, valid_arr, test_arr, stats
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        train_mask = sample_training_modality_mask(modality_mask, modality_dropout_prob)
        outputs = model(
            emb1,
            emb2,
            emb3,
            modality_mask=train_mask,
            task_context=task_context,
            return_aux=aux_loss_weight > 0,
        )
...
def _load_concat_embeddings(embeddings_root: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    graph = load_one("kpgt")
    return load_one("chemberta"), graph
...
def fit_on_task(
    ...
    ept_paths = _resolve_ept_embedding_paths(task_dir)
    ...
    emb_all_s, emb_all_g = _load_concat_embeddings(embeddings_root)
    ...
    emb_tr_3d = np.load(ept_paths["train"]).astype(np.float32)
    emb_va_3d = np.load(ept_paths["valid"]).astype(np.float32)
    emb_te_3d = np.load(ept_paths["test"]).astype(np.float32)

    if emb_tr_3d.shape[0] != n_tr or emb_va_3d.shape[0] != n_va or emb_te_3d.shape[0] != n_te:
        raise ValueError(
            "Maplight split sizes do not match task CSV sizes: "
            f"train={emb_tr_3d.shape[0]}/{n_tr}, valid={emb_va_3d.shape[0]}/{n_va}, test={emb_te_3d.shape[0]}/{n_te}"
        )

    emb_tr_3d, emb_va_3d, emb_te_3d, ept_stats = sanitize_and_normalize_ept(
        emb_tr_3d, emb_va_3d, emb_te_3d
    )
    ...
    if modalities not in modality_map:
        raise ValueError(
            ...
            "Expected one of: all|chemberta|ept|kpgt|chemberta_kpgt|ept_kpgt|chemberta_ept."
        )

    # Model forward expects mask in order: (chemberta/smiles, graph, ept-third-modality)
    modality_mask: Optional[Tuple[bool, bool, bool]] = modality_map[modalities]
    train_dataset = MultiModalDataset(emb_tr_s, emb_tr_3d, emb_tr_g, y_tr, task_type=task_type)
    train_sampler = None
    train_shuffle = True
    if task_type == "classification" and config.use_weighted_sampler:
        sample_weights = np.where(np.asarray(y_tr).astype(int) == 1, float(config.sampler_pos_weight), 1.0).astype(np.float64)
    ...
    if task_type == "classification":
        ...
        use_ap_surrogate = loss_type in {"ap_surrogate", "auprc_surrogate", "soap_surrogate"}
        use_auc_surrogate = loss_type in {"auc_surrogate", "auroc_surrogate", "auc_margin"}
        use_focal_loss = loss_type in {"focal", "focalloss"} or (
            loss_type == "auto" and primary_metric_name == "AUPRC"
        )
        if use_ap_surrogate:
            criterion = AUPRCSurrogateLoss(alpha=class_weights, gamma=float(config.focal_gamma))
        elif use_auc_surrogate:
            criterion = AUROCMarginLoss(alpha=class_weights, label_smoothing=float(config.label_smoothing))
        elif use_focal_loss:
    ...
    for epoch in range(config.max_epochs):
        train_loss = train_one_epoch(
            ...
            modality_dropout_prob=float(getattr(config, "modality_dropout_prob", 0.0)),
        )
        if task_type == "classification":
    ...
    if task_type == "classification":
        ...
        use_ap_surrogate = lt in {"ap_surrogate", "auprc_surrogate", "soap_surrogate"}
        use_auc_surrogate = lt in {"auc_surrogate", "auroc_surrogate", "auc_margin"}
        use_focal_loss = lt in {"focal", "focalloss"} or (
            lt == "auto" and primary_metric_name == "AUPRC"
        )
        if use_ap_surrogate:
            loss_type = "AUPRCSurrogateLoss"
        elif use_auc_surrogate:
            loss_type = "AUROCMarginLoss"
        else:
            loss_type = "FocalLoss" if use_focal_loss else "CrossEntropyLoss"
    else:
        if primary_metric_name == "Spearman":
    ...
    meta = {
        ...
        "device": str(device),
        "seed": config.seed,
        "dims": {"chemberta": int(emb_tr_s.shape[1]), "ept": int(emb_tr_3d.shape[1]), "kpgt": int(emb_tr_g.shape[1])},
