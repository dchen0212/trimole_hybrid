...
class MultiModalFusionMLP(nn.Module):
    def __init__(
        ...
        if self.fusion_type in {"gated_3d_downweight", "gated3d", "gated_3d"}:
            ...
            # Keep the 3D-downweight branch active by interpolating with the base
            # fusion rather than instantiating the module and then multiplying it by 0.
            self.dynamic_alpha_scale = 1.0
    ...
    def forward(
        ...
        if self.residual_dynamic_fusion is not None:
            ...
            if self.fusion_type in {"gated_3d_downweight", "gated3d", "gated_3d"}:
                fused = (1.0 - alpha) * fused_base + alpha * fused_dyn
            else:
