"""Historical CYP-family experiment retained for provenance.
...
``run_family_transfer_leakage_safe_v2.py --family cyp_substrate`` instead.
"""
...
TARGET_META = {
    "cyp2c9_substrate_carbonmangels": ("auprc", 0.474),
    "cyp2d6_substrate_carbonmangels": ("auprc", 0.736),
    "cyp3a4_substrate_carbonmangels": ("auroc", 0.667),
}
SEEDS = [1, 2, 3, 4, 5]
...
def load_dense(data_root: Path, task: str, train: pd.DataFrame, valid: pd.DataFrame, test: pd.DataFrame) -> dict[str, tuple[np.ndarray, np.ndarray, np.ndarray]]:
    task_dir = data_root / task
...
def build_task_features(repo: Path, data_root: Path) -> tuple[dict[str, dict[str, tuple[np.ndarray, np.ndarray, np.ndarray]]], dict[str, dict[str, object]]]:
...
def xgb_grid(y: np.ndarray) -> list[dict[str, float | int]]:
...
def make_model(seed: int, cfg: dict[str, float | int], n_jobs: int) -> XGBClassifier:
...
def pooled_xy(
    ...
    for task in TASKS:
        ...
        y_train = meta[task]["y_train"]
        y_valid = meta[task]["y_valid"]
        xs.append(x_train)
        ys.append(y_train)  # type: ignore[arg-type]
        if include_valid:
            xs.append(x_valid)
            ys.append(y_valid)  # type: ignore[arg-type]
...
def main() -> None:
    ...
    out_root = Path(args.out_root) if args.out_root else repo / "results_strict" / "cyp_substrate_pooled_family_xgb_v1"
    out_root.mkdir(parents=True, exist_ok=True)

    features, meta = build_task_features(repo, data_root)
    rows: list[dict[str, object]] = []
    for feat_name in common_feature_sets(features):
        x_pool_train, y_pool_train = pooled_xy(features, meta, feat_name, include_valid=False)
        x_pool_trainvalid, y_pool_trainvalid = pooled_xy(features, meta, feat_name, include_valid=True)
        for cfg_idx, cfg in enumerate(xgb_grid(y_pool_train)):
            for target in TASKS:
                metric, top1_ref = TARGET_META[target]
                ...
                for seed in SEEDS:
                    ...
                    if valid_path.exists() and test_path.exists() and not args.force:
                        ...
                        train_model.fit(x_pool_train, y_pool_train, eval_set=[(x_valid, y_valid)], verbose=False)
                        vp = train_model.predict_proba(x_valid)[:, 1]
                        final_model = make_model(seed, cfg, args.n_jobs)
                        final_model.fit(x_pool_trainvalid, y_pool_trainvalid, eval_set=[(x_valid, y_valid)], verbose=False)
                        tp = final_model.predict_proba(x_test)[:, 1]
                        write_pred(valid_path, y_valid, vp)
                        write_pred(test_path, y_test, tp)
                    valid_preds.append(vp)
                    test_preds.append(tp)
                    valid_scores.append(score(metric, y_valid, vp))
                    test_scores.append(score(metric, y_test, tp))
                ...
                row = {
                    ...
                    **cfg,
                    ...
                    "valid_ensemble": score(metric, y_valid, np.mean(valid_preds, axis=0)),
                    "test_mean": float(np.nanmean(test_arr)),
                    "test_std": float(np.nanstd(test_arr, ddof=1)),
                    "test_ensemble": score(metric, y_test, np.mean(test_preds, axis=0)),
                    "top1_ref": float(top1_ref),
                    "beats_mean": bool(float(np.nanmean(test_arr)) >= float(top1_ref)),
                    "beats_ensemble": bool(score(metric, y_test, np.mean(test_preds, axis=0)) >= float(top1_ref)),
